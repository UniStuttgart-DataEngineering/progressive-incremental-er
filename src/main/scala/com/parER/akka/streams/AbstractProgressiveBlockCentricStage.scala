...
import com.parER.core.prioritizing.{BlockSizePrioritizer, Prioritizing}
...
class AbstractProgressiveBlockCentricStage(rankMethod: String, progressiveMethod:String, size1: Int, size2: Int = 0, ro: Float = 0.005f, ff: Float = 0.01f, noUpdate: Boolean = false) extends GraphStage[FlowShape[(Int, TokenNGrams), Message]] {
  ...
  override def createLogic(attr: Attributes): GraphStageLogic =
    new GraphStageLogic(shape) {
      ...
      val blockScheduling = new BlockSizePrioritizer(size1, size2, ro, ff)
      ...
      setHandler(in, new InHandler {
        ...
        override def onPush(): Unit = {
          ...
          val (minDid, minProfiles, minBlock)= blockScheduling.progressiveProcess(i, p)
          ...
          // TODO CompGeneration takes profiles to be
          t0 = System.currentTimeMillis()
          var comps = compGeneration.blockComparisons(minDid, minProfiles, minBlock)
...
/*
          ...
          if (comps.size > 0) {
            ...
            // TODO does not work with block prioritizer
//            comps.foreach(x => {
//                if (x.sim - 1.0f > 0.0f) {
...
//          println("count avg comparisons = " + compRanker.getAvgNComparisons())
          ...
          while ( !blockScheduling.isEmpty()) {
            val (minDid, minProfiles, minBlock) = blockScheduling.getMinBlock()
            var comps = compGeneration.blockComparisons(minDid, minProfiles, minBlock)
            //comps = rankingCompCleaner.execute(comps)
            //compRanker.update(comps)
            //comps = compRanker.getBestComparisons()
