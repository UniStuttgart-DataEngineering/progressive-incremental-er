...
import com.parER.core.prioritizing.{HPrioritizer7, LightWeightPQPrioritizer, PQPrioritizer}

class IncrementalComparisonPrioritization(val ignoreComparisonRequest: Boolean = false, val comparisonBudget: Int = 1000) extends GraphStage[FlowShape[Message, Message]] {
...
//            println(s"[HPrioritizer6: OVERHEAD] tCompPrioUpdateFetch (s) = ${tCompPrioUpdateFetch/1000}")
          ...
          val (u, c) = msg match {
            ...
            case MessageComparisons(blockComparisons) =>
              // todo this one makes pruning???
              //println("receive block message... " + K)
