...
class BloomFilter[T] private (val numberOfBits: Long, val numberOfHashes: Int, private val bits: UnsafeBitArray)
  ...
  def expectedFalsePositiveRate(): Float = {
    math.pow(bits.getBitCount.toFloat / numberOfBits, numberOfHashes.toFloat).toFloat
  }
  ...
  def approximateElementCount(): Long = {
    val fractionOfBitsSet = bits.getBitCount.toFloat / numberOfBits
    val x = -log1p(-fractionOfBitsSet) * numberOfBits / numberOfHashes
    val z = rint(x)
    if (abs(x - z) == 0.5f) {
      (x + Math.copySign(0.5f, x)).toLong
    } else {
      z.toLong
    }
  }
...
object BloomFilter {
  ...
  def optimalNumberOfBits(numberOfItems: Long, falsePositiveRate: Float): Long = {
