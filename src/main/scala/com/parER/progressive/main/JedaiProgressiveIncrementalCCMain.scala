...
import com.parER.datastructure.{BaseComparison, Comparison, MessageComparison}
import com.parER.progressive.main.JedaiProgressiveCCMain.{profiles1, profiles2}
import com.parER.utils.{CsvWriter, ExtendedProfileMatcher, JedaiComparisonCleaning, JedaiMatching, JedaiPrioritization}
...
object JedaiProgressiveIncrementalCCMain extends App {
    ...
    val profiles2 = eReader2.getEntityProfiles
    ...
    val chunkSize2 = (profiles2.size().toFloat/N.toFloat).ceil.toInt
    val groupedProfiles2 = profiles2.asScala.grouped(chunkSize2).toList
    ...
    for ( i <- 1 to N) {
        ...
        val prioritization = if (Config.prioritizer.equalsIgnoreCase("efilt" ))
            new EntityFiltering(WeightingScheme.JS, blocks, profiles1.size+profiles2.size) else
