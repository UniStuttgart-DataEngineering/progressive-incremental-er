...
import com.parER.progressive.main.JedaiProgressiveIncrementalDirtyMain.{eFile1, gtFile, profiles1}
import com.parER.utils.{ExtendedProfileMatcher, GtSerializationReaderPartial, JedaiPrioritization, UnilateralDuplicatePropagation}
...
object AkkaJedaiProgressiveIncrementalDirtyMain {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val matcherFlow = Flow.fromGraph(GraphDSL.create() { implicit b =>
      ...
      val matcherStage = b.add( Flow[MessagePrioritization].statefulMapConcat { () =>
        ...
        val pm = new ExtendedProfileMatcher(profiles1.map( el => el._1).toList.asJava, null, RepresentationModel.TOKEN_UNIGRAMS, SimilarityMetric.JACCARD_SIMILARITY)
    ...
    println(s"Increments per $time ms: ${throttleElements.toInt}/s")
    val done =
