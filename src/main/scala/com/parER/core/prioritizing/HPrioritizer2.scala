...
class HPrioritizer2 extends Prioritizing {
  ...
  def refineEmptyH() = {
    println(s"refineEmpty with H(1.0f) empty and |H| = ${H.size}")
    val t0 = System.currentTimeMillis()
    val (k, v) = H.maxBefore(1.0f) match { case Some(p) => p }
    println(s"Adding to H(1.0f) ${v.size} comparisons... from ${k}")
    H(1.0f).addAll(v)
    H.remove(k)
    println("End refineEmpty " + (System.currentTimeMillis()-t0) + " ms")
  }
