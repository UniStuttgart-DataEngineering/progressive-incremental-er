...
import com.parER.utils.GtSerializationReaderPartial
...
object AkkaProgressiveIncrementalCCMain {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //val throttleMode = s"-throttle-"+throttleElements.toInt//+"x"+Config.time
...
//    })
    ...
    val loopFlow = if (Config.prioritizer == "ipbs" || Config.prioritizer == "ipbs2" || Config.prioritizer == "ipbs3" || Config.prioritizer == "ipbs4")
      ...
      Flow.fromGraph(GraphDSL.create() { implicit b =>
        ...
        val tokenBlocker = if (Config.prioritizer == "ipbs")
            b.add(new IncrementalBlockPrioritization(
                ...
                profiles1.size, profiles2.size, Config.cuttingRatio,
                Config.filteringRatio))
        else // case ipbs2 || ipbs3 with weighting and priority queue
        ...
        sourceEmpty ~>
          Flow[Seq[(Int, TokenNGrams)]].takeWhile(_ => {!ignoreComparisonRequests}) ~>
          merge.in(1)

        merge.out ~>
          Flow[Seq[(Int, TokenNGrams)]]
            .takeWhile{_ => (System.currentTimeMillis() - t1 < maxTime || ignoreComparisonRequests)} ~>
          tokenBlocker
        ...
        Flow.fromGraph(GraphDSL.create() { implicit b =>
          ...
          val tokenBlocker = Config.ranker match  {
            case "cbs"   =>  new IncrementalComparisonRanking(Config.ranker, Config.prioritizer, profiles1.size, profiles2.size, Config.cuttingRatio, Config.filteringRatio)
            case "bjs"  =>  new IncrementalComparisonRankingWithBJS(Config.ranker, Config.prioritizer, profiles1.size, profiles2.size, Config.cuttingRatio, Config.filteringRatio)
            case _      =>  new IncrementalComparisonRanking(Config.ranker, Config.prioritizer, profiles1.size, profiles2.size, Config.cuttingRatio, Config.filteringRatio)
          }
...
//      }
      ...
      .via(new StoreSeqModelStageWithIncrCount(profiles1.size, profiles2.size))
